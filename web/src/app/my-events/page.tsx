...
export default function MyEventsPage() {
  ...
  const getMeta = (title: string) => {
    ...
    return EVENT_META[key] || {
      ...
      locationLink: `https://maps.google.com/?q=${encodeURIComponent(title)}`,
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 select-none animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <span className="text-[9px] font-black tracking-widest text-[var(--brand-3)] uppercase text-shadow-glow">My Tickets</span>
          <h1 className="text-xl sm:text-2xl font-black text-white mt-1">Booked Passes</h1>
        </div>

        {/* Tab Switcher */}
        <div className="flex bg-white/5 border border-white/10 rounded-xl p-1 w-fit self-start sm:self-auto">
          <button 
