...
export function NotificationsModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div className="w-full max-w-md rounded-lg liquid-glass liquid-edge border border-white/10 p-5 shadow-card max-h-[80vh] overflow-y-auto animate-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-col gap-3">
          {notifications.map((n) => (
            <div key={n.id} className="p-3 rounded-md bg-white/5 border border-white/5 flex items-start gap-3 justify-between">
              <div>
              ...
              {n.action && (
                <div className="flex gap-1 shrink-0">
                  ...
                  <button className="px-2.5 py-1 rounded-lg bg-white/10 text-white/70 text-[10px] font-bold hover:bg-white/15">
                    Decline
                  </button>
                </div>
              )}
...
export function ProfilePreviewModal({ isOpen, onClose, username }: { isOpen: boolean; onClose: () => void; username: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div className="w-full max-w-sm rounded-lg liquid-glass liquid-edge border border-white/10 p-5 shadow-card animate-in zoom-in-95 duration-200 text-center">
        <div className="flex justify-end">
          <button onClick={onClose} className="p-1 rounded-full hover:bg-white/10 text-white/50 hover:text-white">
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="h-16 w-16 mx-auto rounded-full bg-brand-gradient flex items-center justify-center font-bold text-lg border border-white/10 text-white mb-3">
        ...
        <div className="flex justify-center gap-6 my-4 border-y border-white/5 py-3">
          <div>
            <p className="text-xs text-white/40 uppercase font-semibold">Followers</p>
            <p className="text-sm font-bold text-white">1.2K</p>
          </div>
          <div>
            <p className="text-xs text-white/40 uppercase font-semibold">Events</p>
...
export function EventPreviewModal({ isOpen, onClose, eventTitle }: { isOpen: boolean; onClose: () => void; eventTitle: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div className="w-full max-w-md rounded-lg liquid-glass liquid-edge border border-white/10 p-5 shadow-card animate-in zoom-in-95 duration-200">
...
export function BookingModal({ isOpen, onClose, eventTitle, price }: { isOpen: boolean; onClose: () => void; eventTitle: string; price: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="w-full max-w-sm rounded-lg liquid-glass liquid-edge border border-white/10 p-5 shadow-card animate-in zoom-in-95 duration-200">
        {booked ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-xs text-white/60">Ticket QR is added to your profile widgets.</p>
          </div>
        ) : (
          <>
            <div className="flex justify-between items-start mb-4">
            ...
            {/* Ticket count select */}
            <div className="flex items-center justify-between py-2 px-3 rounded-2xl bg-white/5 border border-white/5 mb-4">
              <span className="text-xs font-bold text-white/70">Number of tickets</span>
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => qty > 1 && setQty(qty - 1)}
                  className="h-8 w-8 rounded-xl bg-white/5 text-white flex items-center justify-center font-black hover:bg-white/10 active:scale-95 transition-all"
                >
                  -
                </button>
            ...
            <div className="space-y-2 mb-6 text-xs text-white/70">
              ...
              <div className="flex justify-between font-bold text-white text-sm border-t border-white/5 pt-2 mt-2">
...
export function CreateEventModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div className="w-full max-w-md rounded-lg liquid-glass liquid-edge border border-white/10 p-5 sm:p-6 shadow-card animate-in zoom-in-95 duration-200">
        {created ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <h3 className="text-lg font-black text-white mb-1">Event Created!</h3>
            <p className="text-xs text-white/60">Your event will show up on nearby feed searches shortly.</p>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-white/50 uppercase tracking-wider mb-1.5">Event Name</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:border-[var(--brand-1)] transition-all placeholder-white/30"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-white/50 uppercase tracking-wider mb-1.5">Category</label>
                  <select
                    value={category}
                    onChange={(e) => setCategory(e.target.value)}
                    className="w-full px-4 py-3 rounded-2xl bg-[#12121a] border border-white/10 text-sm text-white focus:outline-none focus:border-[var(--brand-2)] transition-all"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-white/50 uppercase tracking-wider mb-1.5">Price (₹)</label>
                  <input
                    type="number"
                    placeholder="999"
                    className="w-full px-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:border-[var(--brand-1)] transition-all placeholder-white/30"
