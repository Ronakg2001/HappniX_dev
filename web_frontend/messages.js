(function () {
  ...
  const groupSettingsPanelEl = document.getElementById(
    "message-group-settings-panel",
  );
  const groupSettingsTitleEl = document.getElementById(
    "message-group-settings-title",
  );
  const groupSettingsCloseBtn = document.getElementById(
    "message-group-settings-close-btn",
  );
  const groupSettingsNameInput = document.getElementById(
    "message-group-settings-name",
  );
  const groupSettingsDescriptionInput = document.getElementById(
    "message-group-settings-description",
  );
  const groupSettingsSaveBtn = document.getElementById(
    "message-group-settings-save-btn",
  );
  const groupSettingsSearchInput = document.getElementById(
    "message-group-settings-search",
  );
  const groupSettingsSearchResultsEl = document.getElementById(
    "message-group-settings-search-results",
  );
  const groupSettingsMembersEl = document.getElementById(
    "message-group-settings-members",
  );
  const groupSettingsLeaveBtn = document.getElementById(
    "message-group-settings-leave-btn",
  );
  const groupSettingsDeleteBtn = document.getElementById(
    "message-group-settings-delete-btn",
  );
  const badgeEls = [
  ...
  function renderMessageStatusDot(message) {
    ...
    const stateConfig =
      getFrontendConfigValue(`messaging.statusDot.states.${status}`, null) ||
      fallbackStates[status] ||
      fallbackStates.delivered;
    const label = String(
      stateConfig.label || fallbackStates[status]?.label || "Delivered",
    );
    const className = String(
      stateConfig.className || fallbackStates[status]?.className || "bg-white",
    );
      ...
      label:
        message.localStatus === "failed"
          ...
          : message.localStatus === "sending"
            ? "Sending"
            : "Sent",
      state: message.localStatus || "sent",
      detail: "",
    };
  }
  ...
  function buildOptimisticMessage(conversation, body, options = {}) {
    const localAttachments = serializePendingAttachments(
      options.attachments || [],
    );
    return {
  ...
  function getDynamicOnlineStatus(userOrLastActive) {
    if (userOrLastActive && typeof userOrLastActive === "object") {
      if (typeof userOrLastActive.isOnline === "boolean")
        return userOrLastActive.isOnline;
  ...
  function renderTypingBubble() {
    const conversation = getActiveConversation();
    if (!conversation || !state.typingIndicators[Number(conversation.id)])
      return;
    const container = threadBodyEl.querySelector(
      ".mx-auto.flex.w-full.max-w-3xl.flex-col",
    );
    if (!container || document.getElementById("typing-indicator-bubble"))
      return;
    ...
    state.conversations = state.conversations.map((conversation) => {
      ...
      return {
        ...
        otherUser: {
          ...
          lastActive:
            presence.lastActive || conversation.otherUser?.lastActive || null,
        },
      };
    });
    return changed;
  ...
  function applyPendingDeliveredReceipt(message) {
    ...
    if (!conversationId || !messageId || !message?.isOwn || message.readAt)
      return message;
    ...
    if (!bucket.size)
      delete state.pendingDeliveredByConversation[conversationId];
    return {
      ...message,
  ...
  function applyPendingReadReceipt(message) {
    ...
    if (!Object.keys(bucket).length)
      delete state.pendingReadByConversation[conversationId];
    return {
      ...message,
      readAt,
      localStatus: "read",
  ...
  function conversationDisplayName(conversation) {
    ...
    if (isGroupConversation(conversation))
      return conversation.title || conversation.otherUser?.full_name || "Group";
    const other = conversation.otherUser || {};
  ...
  function renderSearchResults() {
    ...
    searchResultsEl.innerHTML = state.searchResults
      .map((user) => {
        const selected = state.groupBuilder.memberIds.includes(
          Number(user.sql_user_id),
        );
        const action = state.groupBuilder.active
          ? "toggle-group-member"
          : "start-chat";
        const label = state.groupBuilder.active
          ? selected
            ? "Remove"
            : "Add"
          : "Chat";
  ...
  function renderGroupSettingsPanel() {
    ...
    if (!visible || !conversation) {
      if (groupSettingsSearchResultsEl)
        groupSettingsSearchResultsEl.innerHTML = "";
      if (groupSettingsMembersEl) groupSettingsMembersEl.innerHTML = "";
      return;
    }
    if (groupSettingsTitleEl)
      groupSettingsTitleEl.textContent = conversationDisplayName(conversation);
    if (groupSettingsNameInput)
      groupSettingsNameInput.value = conversation.title || "";
    if (groupSettingsDescriptionInput)
      groupSettingsDescriptionInput.value = conversation.description || "";
    if (groupSettingsSaveBtn)
      groupSettingsSaveBtn.classList.toggle(
        "hidden",
        !conversation.permissions?.canRenameGroup,
      );
    if (groupSettingsSearchInput)
      groupSettingsSearchInput.parentElement?.classList.toggle(
        "hidden",
        !conversation.permissions?.canManageMembers,
      );
    if (groupSettingsLeaveBtn)
      groupSettingsLeaveBtn.classList.toggle(
        "hidden",
        !conversation.permissions?.canLeaveGroup,
      );
    if (groupSettingsDeleteBtn)
      groupSettingsDeleteBtn.classList.toggle(
        "hidden",
        !conversation.permissions?.canDeleteGroup,
      );
    ...
    if (groupSettingsMembersEl) {
      groupSettingsMembersEl.innerHTML = (conversation.members || [])
        .map((member) => {
          const canManage =
            Boolean(conversation.permissions?.canManageMembers) &&
            !member.isSelf;
  ...
  async function saveGroupSettings() {
    ...
    try {
      const data = await postJson(
        `/api/messages/groups/${conversationEntityId(conversation)}/rename`,
        {
          name: String(groupSettingsNameInput?.value || "").trim(),
          description: String(
            groupSettingsDescriptionInput?.value || "",
          ).trim(),
        },
      );
  ...
  async function addUsersToActiveGroup(userIds) {
    const conversation = getActiveConversation();
    if (!conversation || !isGroupConversation(conversation) || !userIds?.length)
      return;
    try {
      const data = await postJson(
        `/api/messages/groups/${conversationEntityId(conversation)}/members`,
        {
  ...
  async function updateActiveGroupRole(userId, role) {
    ...
    try {
      ...
      const data = await postJson(
        `/api/messages/groups/${conversationEntityId(conversation)}/members/${Number(userId)}/role`,
        { role: nextRole },
      );
      if (data.conversation) upsertConversation(data.conversation);
      renderConversationList();
  ...
  async function removeUserFromActiveGroup(userId) {
    ...
    const member = (conversation.members || []).find(
      (item) => Number(item.sql_user_id) === Number(userId),
    );
    if (!member) return;
    if (
      !window.confirm(
        `Remove ${member.full_name || member.username} from this group?`,
      )
    )
      return;
    try {
      const data = await postJson(
        `/api/messages/groups/${conversationEntityId(conversation)}/members/${Number(userId)}/remove`,
        {},
      );
      if (data.conversation) upsertConversation(data.conversation);
      renderConversationList();
  ...
  async function leaveActiveGroup() {
    ...
    try {
      await postJson(
        `/api/messages/groups/${conversationEntityId(conversation)}/leave`,
        {},
      );
  ...
  async function runGroupMemberSearch(query) {
    ...
    try {
      const data = await getJson(
        `/api/users/search?q=${encodeURIComponent(value)}&limit=8`,
      );
      const existingIds = new Set(
        (conversation.members || []).map((member) =>
          Number(member.sql_user_id || 0),
        ),
      );
      state.groupSettings.searchResults = (
        ...
        (user) =>
          user &&
          user.sql_user_id &&
          !existingIds.has(Number(user.sql_user_id)),
      );
    } catch (_error) {
      state.groupSettings.searchResults = [];
    }
    ...
    composePreviewItemsEl.innerHTML = items
      .map((item) => {
        const name = escapeHtml(
          item.file?.name || `${item.kind || "file"} attachment`,
        );
        const meta = [];
        meta.push(
          item.kind === "audio"
            ? "Voice note"
            : (item.kind || "file").toUpperCase(),
        );
  ...
  function renderSpecialMessageBody(message) {
    ...
    const footer =
      lines.find((line) => /^open my events/i.test(line)) ||
  ...
  function renderMessageAttachments(message) {
    ...
    return `<div class="mt-3 space-y-2">${attachments
      .map((attachment) => {
        const kind = attachment.kind || attachment.type || "file";
        const name = escapeHtml(
          attachment.name || attachment.file?.name || `${kind} attachment`,
        );
        ...
        if (attachment.durationSeconds)
          metaBits.push(`${attachment.durationSeconds}s`);
  ...
  function toggleGroupMember(userId) {
    ...
    if (!id) return;
    const index = state.groupBuilder.memberIds.findIndex(
      (item) => Number(item) === id,
    );
    if (index >= 0) {
      ...
      const user = state.searchResults.find(
        (item) => Number(item.sql_user_id) === id,
      );
      if (!user) return;
  ...
  async function submitMessagePayload(options = {}) {
    ...
    const attachmentItems = serializePendingAttachments(
      options.attachments || [],
    );
    ...
    try {
      ...
      if (attachmentItems.length) {
        ...
        data = await postMultipart(
          conversationMessagesPath(conversation),
          formData,
        );
      } else {
  ...
  async function deleteMessageForMe(messageId) {
    ...
    try {
      const data = await postJson(
        deleteMessagePath(getMessageById(messageId)),
        {},
      );
      state.openMessageMenuId = null;
  ...
  async function unsendMessage(messageId) {
    ...
    try {
      const data = await postJson(
        unsendMessagePath(getMessageById(messageId)),
        {},
      );
      state.openMessageMenuId = null;
      replaceMessageInCache(data.message);
  ...
  async function handleSocketPayload(payload) {
    ...
    if (payload.type === "conversation.updated" && payload.conversation) {
      ...
      if (
        ...
        typeof triggerNotificationPop === "function"
      ) {
    ...
    if (
      ...
      console.log("SOCKET: New message received ->", payload.message);
      ...
      if (isVisibleActiveConversation && isIncomingMessage) {
        ...
        const latestMessage = (
          state.messagesByConversation[conversationId] || []
        ).find((item) => Number(item.id) === Number(payload.message.id));
        if (latestMessage) {
      ...
      if (!payload.message.isOwn) {
        console.log(
          "SOCKET: Message is from someone else. Attempting animation...",
        );

        // 3. Try to trigger the animation
        if (typeof window.triggerNotificationPop === "function") {
          console.log("SOCKET: Animation function found! Triggering neon pop.");
          window.triggerNotificationPop();
        } else {
          console.error(
            "SOCKET ERROR: Cannot find window.triggerNotificationPop!",
          );
        }
      } else {
        console.log("SOCKET: Message was sent by me. Skipping animation.");
      }
      if (
    ...
    if (payload.type === "conversation.read") {
      ...
      const conversation = state.conversations.find(
        (item) => Number(item.id) === convoId,
      );
      const isOtherReader =
        conversation &&
        Number(conversation.otherUser?.sql_user_id || 0) === readerUserId;
      if (isOtherReader) {
    ...
    if (payload.type === "messages.read.receipt") {
      ...
      messages.forEach((msg) => {
        const matchesReceipt =
          !readIds.length || readIds.includes(Number(msg.id));
    ...
    if (payload.type === "messages.delivered.receipt") {
      const convoId = Number(payload.conversationId);
      const deliveredIds = (payload.messageIds || [])
        .map((id) => Number(id))
        .filter(Boolean);
    ...
    if (
      payload.type === "group.messages.delivered.receipt" ||
      payload.type === "group.messages.read.receipt"
    ) {
      const convoId = Number(payload.conversationId);
      ...
      messages.forEach((msg) => {
        ...
        const member = (conversation?.members || []).find(
          (item) => Number(item.sql_user_id) === recipientUserId,
        );
        if (!member) return;
        const deliveredIndex = deliveredTo.findIndex(
          (item) => Number(item.sql_user_id) === recipientUserId,
        );
        ...
        if (deliveredIndex >= 0)
          deliveredTo[deliveredIndex] = {
            ...deliveredTo[deliveredIndex],
            deliveredAt: stamp,
          };
        else deliveredTo.push(deliveredEntry);
        if (isRead) {
          const readIndex = readBy.findIndex(
            (item) => Number(item.sql_user_id) === recipientUserId,
          );
          if (readIndex >= 0)
            readBy[readIndex] = {
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (action === "info-message" && actionEl.dataset.messageId) {
      ...
      if (message) {
        ...
        if (message.conversationKind === "group") {
          const deliveredTo =
            Array.isArray(message.deliveredTo) && message.deliveredTo.length
              ? message.deliveredTo
                  .map(
                    (item) =>
                      `${item.full_name || item.username} (${item.deliveredAt ? new Date(item.deliveredAt).toLocaleString() : "pending"})`,
                  )
                  .join("\n")
              : "No one yet";
          const readBy =
            Array.isArray(message.readBy) && message.readBy.length
              ? message.readBy
                  .map(
                    (item) =>
                      `${item.full_name || item.username} (${item.readAt ? new Date(item.readAt).toLocaleString() : "pending"})`,
                  )
                  .join("\n")
              : "No one yet";
...
Read: ${readTime}`);
    ...
    if (
      action === "open-group-settings" ||
      action === "rename-group" ||
      action === "add-group-members" ||
      action === "manage-group-member"
    ) {
    ...
    if (
      action === "group-settings-role" &&
      actionEl.dataset.userId &&
      actionEl.dataset.role
    ) {
      await updateActiveGroupRole(
        actionEl.dataset.userId,
        actionEl.dataset.role,
      );
      return;
  ...
  groupSettingsSearchInput?.addEventListener("input", () => {
    if (state.groupSettings.searchTimer)
      window.clearTimeout(state.groupSettings.searchTimer);
