...
function getFrontendConfigValue(path, fallbackValue) {
  if (
    ...
    typeof window.HappnixFrontendConfig.get === "function"
  ) {
...
function normalizeEventLookupKey(eventId) {
  const raw = String(eventId || "").trim();
  if (!raw) return "";
  return raw.startsWith("event-") ? raw.slice(6) : raw;
}
...
function getActiveTicketForEvent(eventId) {
  const safeEventId = normalizeEventLookupKey(eventId);
  ...
  return (
    (state.tickets || []).find((ticket) => {
      ...
      if (normalizeEventLookupKey(ticket.event?.id) !== safeEventId) return false;
...
function openProfileEditor() {
  ...
  state.profileEditor.avatarUrl = isPlaceholderAvatar(state.currentUser.avatar)
    ? ""
    : state.currentUser.avatar || "";
...
function isEventCancelledFromDetails(eventDetails) {
  const rawStatus = String(eventDetails?.status || "")
    .trim()
    .toLowerCase();
...
function pluralizeCountdownUnit(unitKey, count) {
  const singular = getFrontendConfigValue(
    `events.countdown.unitLabels.${unitKey}`,
    unitKey,
  );
  const plural = getFrontendConfigValue(
    `events.countdown.unitLabels.${unitKey}s`,
    `${singular}s`,
  );
...
function formatRelativeCountdown(parts) {
  ...
  if (!body)
    return getFrontendConfigValue(
      "events.countdown.fallbackDateText",
      "Date TBD",
    );
...
function formatEventEndCountdownLabel(eventDetails, now = new Date()) {
  ...
  if (days <= 0) {
    return formatConfiguredPattern(
      getFrontendConfigValue(
        "events.liveProgress.endsSameDayPattern",
        "Ends in {time}",
      ),
      { time: timeText },
    );
  ...
  return formatConfiguredPattern(
    getFrontendConfigValue(
      "events.liveProgress.endsMultiDayPattern",
      "Ends in {days} day{daySuffix} {time}",
    ),
    {
      days,
      daySuffix: days === 1 ? "" : "s",
...
function getLiveEventProgressMeta(eventDetails, now = new Date()) {
  ...
  const endingSoonThreshold =
    Number(
      getFrontendConfigValue("events.liveProgress.endingSoonThreshold", 0.7),
    ) || 0.7;
  const endTimerThreshold =
    Number(
      getFrontendConfigValue("events.liveProgress.endTimerThreshold", 0.85),
    ) || 0.85;
  ...
  if (progress >= endingSoonThreshold) {
    return {
      detail: getFrontendConfigValue(
        "events.liveProgress.endingSoonText",
        "Event will end soon",
      ),
    };
  }
...
function getUpcomingEventCountdownLabel(eventDetails, now = new Date()) {
  const startDate = getEventStartDate(eventDetails);
  const fallbackDateText = getFrontendConfigValue(
    "events.countdown.fallbackDateText",
    "Date TBD",
  );
  ...
  if (dayDiff > 1 && dayDiff <= 3) {
    return formatConfiguredPattern(
      getFrontendConfigValue("events.countdown.daysPattern", "In {count} days"),
      { count: dayDiff },
    );
  }
  ...
  if (
    ...
    nowWeekStart &&
    nowWeekEnd &&
    startDay >= nowWeekStart &&
    startDay <= nowWeekEnd &&
    dayDiff > 3
  ) {
    return isWeekendDate(startDate)
      ? getFrontendConfigValue(
          "events.countdown.thisWeekendLabel",
          "This weekend",
        )
  ...
  if (dayDiff > 7) {
    ...
    if (startDate.getFullYear() > now.getFullYear() + 1) {
      return formatRelativeCountdown([
        distance.years > 0
          ? pluralizeCountdownUnit("year", distance.years)
          : "",
        distance.months > 0
          ? pluralizeCountdownUnit("month", distance.months)
          : "",
    ...
    if (startDate.getFullYear() === now.getFullYear() + 1) {
      return getFrontendConfigValue(
        "events.countdown.nextYearLabel",
        "Next year",
      );
    }
    ...
    if (monthDiff === 1) {
      return getFrontendConfigValue(
        "events.countdown.nextMonthLabel",
        "Next month",
      );
    }
    if (monthDiff === 0) {
      return getFrontendConfigValue(
        "events.countdown.thisMonthLabel",
        "This month",
      );
    }
    ...
    return formatConfiguredPattern(
      getFrontendConfigValue(
        "events.countdown.weeksPattern",
        "In {count} week{suffix}",
      ),
      { count: weeks, suffix: weeks === 1 ? "" : "s" },
    );
  }
  if (dayDiff > 3) {
    const weeks = Math.max(1, Math.ceil(dayDiff / 7));
...
function getEventLifecycleMeta(eventDetails) {
  ...
  const activeKey =
    Object.keys(stateMap).find((key) => stateMap[key]) || "upcoming";
  const stateConfig =
    getFrontendConfigValue(`events.status.states.${activeKey}`, {}) || {};
...
function getEventPostById(postId) {
  ...
  const profilePosts = Object.values(
...
function updateEventStatusBlocks() {
  document
    .querySelectorAll("[data-event-status-block][data-post-id]")
    .forEach((node) => {
      const post = getEventPostById(node.dataset.postId);
      if (!post?.isEvent) return;
      node.outerHTML = renderEventLifecycleStatus(
        node.dataset.postId,
        post.eventDetails,
        {
...
function startEventStatusTicker() {
  ...
  state.eventStatusTimerId = window.setInterval(
    ...
    Number(getFrontendConfigValue("events.statusTickerIntervalMs", 1000)) ||
      1000,
  );
}
  ...
  listEl.innerHTML = items
    .map((item) => {
      ...
      const isMessageNotification =
        String(item.activity_type || "").toLowerCase() === "message";
...
function getLocationSettingsSummary() {
  ...
  if (!state.locationEnabled) {
    return {
      status: "Off",
      detail:
        "Live and nearby party suggestions are paused until you enable location again.",
    };
  }
  ...
  return {
    ...
    detail:
      "Location can be detected from this device whenever permission is granted.",
  };
}
...
function buildSettingsSummaryChips() {
  return [
    {
      label: "Trust",
      value: state.currentUser.govIdVerified
        ? "Verified"
        : "Verification needed",
    },
    {
...
function buildSettingsSections() {
  ...
  const ordersCount = Number(
    state.currentUser.ordersCount || state.tickets.length || 0,
  );
  const activeOrdersCount = Number(
    state.currentUser.activeOrdersCount || state.tickets.length || 0,
  );
  const archivedOrdersCount = Number(
    state.currentUser.archivedOrdersCount || 0,
  );
  ...
  return [
    {
      ...
      description:
        "Core trust, authentication and access control for your Happnix identity.",
      items: [
        {
          title: "Aadhaar / Identity Verification",
          status: state.currentUser.govIdVerified
            ? "Verified"
            : "Action needed",
        ...
        },
        {
          ...
          tone: "warning",
          detail:
            "Add OTP-based second step next to password login for stronger account security.",
        },
        {
          title: "Account Status",
          status:
            state.currentUser.accountStatus === "verified"
              ? "Verified ready"
              : "Limited access",
          tone:
            state.currentUser.accountStatus === "verified"
        ...
        },
        {
          ...
          tone: "neutral",
          detail:
            "Connected surfaces should only receive sign-in, session and core profile access.",
        },
      ],
    },
    {
      ...
      description:
        "Social-first controls for visibility, interactions and safer discovery.",
      items: [
        {
          ...
          actionLabel: state.currentUser.isPrivate
            ? "Make public"
            : "Make private",
        },
        {
        ...
        },
        {
          ...
          tone:
            Number(state.currentUser.blockedCount || 0) > 0
              ? "warning"
              : "neutral",
          detail:
            "People you block will not be able to discover, message or follow you.",
        },
        {
          title: "Restricted Accounts",
          status: `${Number(state.currentUser.restrictedCount || 0)} restricted`,
          tone:
            Number(state.currentUser.restrictedCount || 0) > 0
              ? "warning"
              : "neutral",
          detail:
            "Use restrictions for low-friction safety without a full block.",
        },
        {
          title: "Tags & Mentions",
          ...
          tone: "neutral",
          detail:
            "Control who can tag or mention you in party conversations and social moments.",
        },
        {
          ...
          tone: "neutral",
          detail:
            "Your reactions help Happnix tune discovery, relevance and social recommendations.",
        },
        {
          title: "Location, Live",
    ...
    },
    {
      ...
      description:
        "Supervision, household sharing and access tiers for safe multi-user usage.",
      items: [
        {
          title: "Admin User Access",
          status: roleMeta.label,
          tone:
            state.currentUser.accountRole === "member" ? "neutral" : "success",
          detail:
            "Access changes based on moderation responsibility and account trust level.",
        },
        {
          ...
          tone: "neutral",
          detail:
            "Family-aware roles can support supervised discovery, approvals and shared visibility.",
        },
        {
          ...
          tone: "neutral",
          detail:
            "A future family center can manage child safety, check-ins and content boundaries.",
        },
        {
          ...
          tone: "neutral",
          detail:
            "Shared family plans can reuse approved event posts across linked household accounts.",
        },
        {
          title: "Dashboard",
          status: `${Number(state.currentUser.hostedEventsCount || 0)} hosted`,
          tone:
            Number(state.currentUser.hostedEventsCount || 0) > 0
              ? "success"
              : "neutral",
          detail:
            "Track your hosted events, party activity and attendance performance from your main flow.",
        ...
        },
        {
          ...
          tone:
            Number(state.currentUser.hostedEventsCount || 0) > 0
              ...
              : "neutral",
          detail:
            "Sponsored party boosts and promo spends can live here for hosts and admins.",
        },
      ],
    },
    {
      ...
      description:
        "Profile presentation, saved history, orders and device-side personalization.",
      items: [
        {
          title: "Edit Profile",
          status: "Profile tab",
          tone: "neutral",
          detail:
            "Update your vibe, avatar and social presentation from your personal profile area.",
        ...
        },
        {
          ...
          detail:
            ...
            "Tell people what kind of parties, music or energy you bring.",
        },
        {
          title: "Saved",
          status: `${Number(state.currentUser.savedProfilesCount || 0)} saved`,
          tone:
            Number(state.currentUser.savedProfilesCount || 0) > 0
              ? "success"
              : "neutral",
          detail:
            "Use this space for bookmarked parties, creators and inspiration later.",
        },
        {
          title: "Archive",
          ...
          detail:
            "Old bookings, past party memories and archived tickets can be stored cleanly here.",
        },
        {
        ...
        },
        {
          ...
          tone: "neutral",
          detail:
            "Switch the interface mood between bright clarity and late-night party contrast.",
    ...
    },
    {
      ...
      description:
        "Device access, feedback routes and important policy surfaces.",
      items: [
        {
          ...
          tone: "neutral",
          detail:
            "Happnix is ready to expand into regional and multilingual party communities.",
        },
        {
          ...
          tone: state.locationEnabled ? "success" : "warning",
          detail:
            "Browser-level location, camera and microphone permissions shape discovery, media and safety flows.",
          action: "open-location-settings",
          actionLabel: "Review",
        },
        {
          ...
          tone: "neutral",
          detail:
            "Feedback should collect bug reports, trust concerns and party-quality ideas from the community.",
        },
        {
          title: "Legal & Terms",
          status: "Available",
          tone: "neutral",
          detail:
            "Policies, community standards and payment terms belong here for transparency.",
        },
        {
          title: "Logout",
          ...
          tone: "danger",
          detail:
            "End this session cleanly on shared or low-trust devices whenever needed.",
          action: "logout",
  ...
  switch (category) {
    ...
    case "restricted":
      return {
        ...
        empty: "No restricted accounts yet.",
      };
    default:
      return {
        label: "Saved",
...
function syncSettingsCountsFromLists() {
  state.currentUser.savedProfilesCount = (
    state.settings.people.lists.saved || []
  ).length;
  state.currentUser.blockedCount = (
    state.settings.people.lists.blocked || []
  ).length;
  state.currentUser.restrictedCount = (
    state.settings.people.lists.restricted || []
  ).length;
}
...
function renderSettingsPreferencesControls() {
  ...
  tagsEl.value =
    state.settings.preferences.tagsAndMentionsPermission || "everyone";
  ...
  saveEl.classList.toggle(
    "pointer-events-none",
    !!state.settings.preferences.saving,
  );
}
...
function renderSettingsPeopleManager() {
  ...
  if (state.settings.people.searching) {
    resultsEl.innerHTML =
      '<div class="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-5 py-5 text-sm text-gray-400">Searching people...</div>';
  } else if (
    (state.settings.people.query || "").trim() &&
    state.settings.people.searchResults.length === 0
  ) {
    resultsEl.innerHTML =
      '<div class="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-5 py-5 text-sm text-gray-500">No matching people found.</div>';
  } else {
    resultsEl.innerHTML = state.settings.people.searchResults
      .map((user) => {
        const isBusy =
          Number(state.settings.people.addingId || 0) ===
  ...
  if (state.settings.people.loading) {
    listEl.innerHTML =
      '<div class="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-5 py-6 text-sm text-gray-400">Loading list...</div>';
    return;
  }
  if (list.length === 0) {
  ...
  listEl.innerHTML = list
    .map((user) => {
      const isBusy =
        Number(state.settings.people.removingId || 0) ===
        Number(user.sql_user_id || 0);
      return `
        <article class="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4 shadow-lg shadow-black/10">
          <div class="flex items-center gap-3">
...
async function loadSettingsPeople(
  category = state.settings.people.activeCategory,
  options = {},
) {
  ...
  try {
    ...
    state.settings.people.lists[safeCategory] = Array.isArray(data?.users)
      ? data.users
      : [];
...
async function searchSettingsPeople(query) {
  ...
  try {
    const data = await getJson(
      `/api/users/search?q=${encodeURIComponent(trimmed)}&limit=8`,
    );
    const currentList =
      state.settings.people.lists[state.settings.people.activeCategory] || [];
    const existingIds = new Set(
      currentList.map((user) => Number(user.sql_user_id || 0)),
    );
...
async function addSettingsPerson(category, userId) {
  ...
  try {
    ...
    state.settings.people.searchResults =
      state.settings.people.searchResults.filter(
    ...
    if (category === "blocked") {
      state.settings.people.lists.saved = (
        state.settings.people.lists.saved || []
      ).filter((user) => Number(user.sql_user_id || 0) !== safeUserId);
      state.settings.people.lists.restricted = (
        state.settings.people.lists.restricted || []
      ).filter((user) => Number(user.sql_user_id || 0) !== safeUserId);
      syncSettingsCountsFromLists();
...
async function loadNotifications(options = {}) {
  ...
  try {
    ...
    if (newUnreadCount > state.unreadNotificationsCount) {
      // Only trigger if the function exists (it's globally scoped from mobile_navigation.js)
      if (typeof triggerNotificationPop === "function") {
        triggerNotificationPop();
      }
    }
    state.unreadNotificationsCount = newUnreadCount;
...
async function loadCurrentUserProfile() {
  try {
    ...
    state.currentUser.savedProfilesCount = Number(
      profile.saved_profiles_count || 0,
    );
    ...
    setLocationStatus(error.message || "Failed to load your profile.", true);
  }
  renderCurrentUserProfile();
  renderProfileGrid();
  ...
  document.addEventListener("click", (event) => {
    ...
    switch (action) {
      ...
      case "confirm-booking":
        confirmBooking();
        break;
      case "generate-guest-invite-link":
        generateGuestInviteLink();
        break;
      case "copy-guest-invite-link":
        copyGuestInviteLink();
        break;
      case "share-guest-invite-link":
        shareGuestInviteLink();
        break;
      case "cancel-ticket":
...
function normalizeStoredTicket(ticket) {
  ...
  const guestInvites = Array.isArray(ticket.guestInvites)
    ? ticket.guestInvites.map((guest) => ({
        id: Number(guest.id || 0) || null,
        inviteToken: String(guest.inviteToken || "").trim(),
        fullName: String(guest.fullName || "").trim(),
        mobileNumber: String(guest.mobileNumber || "").trim(),
        email: String(guest.email || "").trim(),
        inviteStatus: String(guest.inviteStatus || "invited").trim().toLowerCase(),
        paymentStatus: String(guest.paymentStatus || "PENDING").trim(),
        paidByUserId: Number(guest.paidByUserId || 0) || null,
        paidByGuest: Boolean(guest.paidByGuest),
        inviteLink: toAbsoluteAppUrl(
          String(
            guest.inviteLink ||
              (guest.inviteToken ? `/guest-invite/${guest.inviteToken}/` : ""),
          ).trim(),
        ),
        ticketUrl: toAbsoluteAppUrl(
          String(
            guest.ticketUrl ||
              (guest.inviteToken ? `/guest-ticket/${guest.inviteToken}/` : ""),
          ).trim(),
        ),
        canPay: Boolean(guest.canPay),
        canCancel: Boolean(guest.canCancel),
      }))
    : [];
  return {
    id: String(
...
function buildBookingParticipantTicketPayload(event) {
  ...
  (bookingDraftState.invitees || []).forEach((user) => {
    const tier =
      tiers[getInviteeTierIndex(user.userId)] || selfTier || tiers[0];
    if (!tier) return;
...
function bookingEventNumericId() {
  return Number(normalizeEventLookupKey(currentBookingEventId));
}
function canCurrentUserInviteGuest(event, existingTicket) {
  const currentUserId = Number(state.currentUser.id || 0);
  return (
    Number(event?.userId || 0) === currentUserId ||
    Boolean(existingTicket && existingTicket.status !== "cancelled")
  );
}
function bookingGuestInviteCardMarkup() {
  const event = getPostById(currentBookingEventId);
  const existingTicket = getActiveTicketForEvent(currentBookingEventId);
  const canInvite = canCurrentUserInviteGuest(event, existingTicket);
  const helperText = canInvite
    ? Number(event?.userId || 0) === Number(state.currentUser.id || 0)
      ? "As the host, you can invite someone outside HappniX even before booking."
      : "You already hold this event ticket, so you can share a guest invite link now."
    : "Book your own ticket first to unlock external guest invites.";
  return `<div id="booking-guest-invite-card" class="rounded-3xl border border-cyan-400/20 bg-cyan-500/10 p-4 space-y-3"><div class="flex items-start justify-between gap-3"><div><div class="text-sm font-semibold text-white">Invite Guest</div><div class="text-xs text-cyan-100/80">${helperText}</div></div><button type="button" id="booking-generate-guest-invite-btn" data-action="generate-guest-invite-link" class="rounded-2xl border border-cyan-300/30 bg-cyan-400/15 px-4 py-2 text-xs font-bold text-cyan-100 hover:bg-cyan-400/25 transition-colors" ${canInvite ? "" : "disabled"}>Generate Link</button></div><div id="booking-guest-invite-status" class="hidden rounded-2xl border border-white/10 bg-slate-950/40 px-3 py-3 text-xs text-gray-200"></div><div id="booking-guest-invite-actions" class="hidden space-y-3"><div class="rounded-2xl border border-white/10 bg-slate-950/70 px-3 py-3 text-xs text-cyan-100 break-all" id="booking-guest-invite-link"></div><div class="flex gap-2"><button type="button" data-action="copy-guest-invite-link" class="flex-1 rounded-2xl border border-white/10 bg-white/5 px-3 py-3 text-xs font-semibold text-white hover:border-cyan-400/40 hover:text-cyan-100 transition-colors">Copy Link</button><button type="button" id="booking-share-guest-invite-btn" data-action="share-guest-invite-link" class="flex-1 rounded-2xl border border-fuchsia-400/30 bg-fuchsia-500/10 px-3 py-3 text-xs font-semibold text-fuchsia-100 hover:bg-fuchsia-500/20 transition-colors">Share</button></div></div></div>`;
}
function updateGuestInviteUi() {
  const actionButton = document.getElementById("booking-generate-guest-invite-btn");
  const statusEl = document.getElementById("booking-guest-invite-status");
  const actionsEl = document.getElementById("booking-guest-invite-actions");
  const linkEl = document.getElementById("booking-guest-invite-link");
  const shareButton = document.getElementById("booking-share-guest-invite-btn");
  if (!actionButton || !statusEl || !actionsEl || !linkEl) return;
  const event = getPostById(currentBookingEventId);
  const existingTicket = getActiveTicketForEvent(currentBookingEventId);
  const canInvite = canCurrentUserInviteGuest(event, existingTicket);
  actionButton.disabled = bookingDraftState.guestInviteLoading || !canInvite;
  actionButton.classList.toggle("opacity-60", bookingDraftState.guestInviteLoading);
  actionButton.textContent = bookingDraftState.guestInviteLoading
    ? "Generating..."
    : bookingDraftState.guestInviteLink
      ? "Generate New Link"
      : "Generate Link";

  if (bookingDraftState.guestInviteError) {
    statusEl.classList.remove("hidden");
    statusEl.className =
      "rounded-2xl border border-rose-400/20 bg-rose-500/10 px-3 py-3 text-xs text-rose-100";
    statusEl.textContent = bookingDraftState.guestInviteError;
  } else if (bookingDraftState.guestInviteLoading) {
    statusEl.classList.remove("hidden");
    statusEl.className =
      "rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-3 py-3 text-xs text-cyan-100";
    statusEl.textContent = "Creating a guest invite link for this event...";
  } else if (bookingDraftState.guestInviteLink) {
    statusEl.classList.remove("hidden");
    statusEl.className =
      "rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-3 py-3 text-xs text-emerald-100";
    statusEl.textContent =
      "Guest link ready. Share it with someone outside HappniX so they can finish onboarding.";
  } else {
    statusEl.classList.add("hidden");
    statusEl.textContent = "";
  }

  if (bookingDraftState.guestInviteLink) {
    actionsEl.classList.remove("hidden");
    linkEl.textContent = bookingDraftState.guestInviteLink;
  } else {
  ...
  if (shareButton) {
    shareButton.disabled =
      !bookingDraftState.guestInviteLink || typeof navigator.share !== "function";
    shareButton.classList.toggle("opacity-60", shareButton.disabled);
  }
}
function mountGuestInviteSection() {
  const totalRow = document
    .getElementById("booking-total-amount")
    ?.closest(".border-t");
  if (!totalRow) return;
  const existing = document.getElementById("booking-guest-invite-card");
  if (!existing) {
    totalRow.insertAdjacentHTML("beforebegin", bookingGuestInviteCardMarkup());
  }
  updateGuestInviteUi();
}
...
async function generateGuestInviteLink() {
  const event = getPostById(currentBookingEventId);
  const eventId = bookingEventNumericId();
  const existingTicket = getActiveTicketForEvent(currentBookingEventId);
  if (!event || !event.eventDetails || !Number.isFinite(eventId) || eventId <= 0) {
    setLocationStatus("Unable to generate a guest invite for this event.", true);
    return;
  }
  if (!canCurrentUserInviteGuest(event, existingTicket)) {
    setLocationStatus("Book your own ticket first to invite a guest.", true);
    return;
  }
  if (bookingDraftState.guestInviteLoading) return;
  bookingDraftState.guestInviteLoading = true;
  bookingDraftState.guestInviteError = "";
  updateGuestInviteUi();
  try {
    const payload = { eventId };
    if (existingTicket?.id) payload.parentTicketId = Number(existingTicket.id);
    const response = await postJson("/api/guest-invites", payload);
    bookingDraftState.guestInviteLink = String(
      response.inviteLink || response.guestInvite?.inviteLink || "",
    ).trim();
    bookingDraftState.guestInviteToken = String(
      response.guestInvite?.inviteToken || "",
    ).trim();
    bookingDraftState.guestInviteError = bookingDraftState.guestInviteLink
      ? ""
      : "The invite link could not be created.";
  } catch (error) {
    bookingDraftState.guestInviteError =
      error.message || "Failed to generate the guest invite link.";
  } finally {
    bookingDraftState.guestInviteLoading = false;
    updateGuestInviteUi();
  }
}
async function copyGuestInviteLink() {
  const link = String(bookingDraftState.guestInviteLink || "").trim();
  if (!link) return;
  try {
    await copyTextValue(link);
    setLocationStatus("Guest invite link copied.");
  } catch (_error) {
    setLocationStatus("Could not copy the guest invite link.", true);
  }
}
async function shareGuestInviteLink() {
  const link = String(bookingDraftState.guestInviteLink || "").trim();
  const event = getPostById(currentBookingEventId);
  if (!link) return;
  if (typeof navigator.share !== "function") {
    await copyGuestInviteLink();
    return;
  }
  try {
    await navigator.share({
      title: event?.eventDetails?.title || "HappniX Guest Invite",
      text: `Join me at ${event?.eventDetails?.title || "this event"} on HappniX.`,
      url: link,
    });
  } catch (_error) {}
}
async function payGuestInviteByOwner(ticketId, inviteToken) {
  try {
    await postJson(`/api/guest-invites/${inviteToken}/pay-by-owner`, {});
    await loadTickets();
    openTicketDetailsModal(ticketId);
  } catch (error) {
    setLocationStatus(
      error.message || "Failed to pay for this guest ticket.",
      true,
    );
  }
}
async function cancelGuestInviteByOwner(ticketId, inviteToken) {
  try {
    await postJson(`/api/guest-invites/${inviteToken}/cancel-by-owner`, {});
    await loadTickets();
    openTicketDetailsModal(ticketId);
  } catch (error) {
    setLocationStatus(
      error.message || "Failed to cancel this guest ticket.",
      true,
    );
  }
}
async function generateGuestInviteForTicket(ticketId) {
  const ticket = (state.tickets || []).find(
    (entry) => String(entry.id) === String(ticketId || ""),
  );
  if (!ticket) {
    setLocationStatus("Ticket not found for guest invite.", true);
    return;
  }
  try {
    const response = await postJson("/api/guest-invites", {
      eventId: Number(normalizeEventLookupKey(ticket.event?.id)),
      parentTicketId: Number(ticket.id),
    });
    const inviteLink = toAbsoluteAppUrl(
      String(
        response.inviteLink ||
          response.guestInvite?.inviteLink ||
          (response.guestInvite?.inviteToken
            ? `/guest-invite/${response.guestInvite.inviteToken}/`
            : ""),
      ).trim(),
    );
    await loadTickets();
    openTicketDetailsModal(ticketId);
    if (inviteLink) {
      await copyTextValue(inviteLink);
      setLocationStatus("Guest invite link created and copied.");
    } else {
      setLocationStatus("Guest invite link created.");
    }
  } catch (error) {
    setLocationStatus(
      error.message || "Failed to generate the guest invite link.",
...
function renderGuestInviteSection(ticket) {
  const canInvite = canCurrentUserInviteGuest(ticket?.event, ticket);
  const guests = Array.isArray(ticket?.guestInvites) ? ticket.guestInvites : [];
  const helperText = canInvite
    ? "Share a link for someone outside HappniX and track their payment here."
    : "Guest invites unlock after you book your own ticket.";
  const inviteAction = canInvite
    ? `<button type="button" data-action="generate-ticket-guest-invite" data-ticket-id="${ticket.id}" class="rounded-xl border border-cyan-400/30 bg-cyan-500/10 px-3 py-2 text-xs font-bold text-cyan-100 hover:bg-cyan-500/20 transition-colors">Invite Guest</button>`
    : "";
  const cardsMarkup = guests.length
    ? renderGuestInviteCards(ticket)
    : `<div class="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-4 py-4 text-sm text-gray-500">No guest invites have been attached to this ticket yet.</div>`;
  return `<div class="space-y-3"><div class="flex items-start justify-between gap-3"><div><div class="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-500">Guest Invites</div><div class="mt-1 text-xs text-gray-400">${helperText}</div></div>${inviteAction}</div>${cardsMarkup}</div>`;
}
function renderGuestInviteCards(ticket) {
  const guests = Array.isArray(ticket?.guestInvites) ? ticket.guestInvites : [];
  return guests
    .map((guest) => {
      const isPending =
        String(guest.paymentStatus || "").toUpperCase() === "PENDING";
      const isCancelled =
        String(guest.inviteStatus || "").toLowerCase() === "cancelled";
      ...
      const badgeLabel = isCancelled
        ...
        : isPending
          ? "Pending Payment"
          : "Confirmed";
      return `<div class="rounded-2xl border border-white/10 bg-white/[0.03] p-4 space-y-3"><div class="flex items-start justify-between gap-3"><div><div class="font-semibold text-white">${escapeHtml(guest.fullName || "Guest invite")}</div><div class="mt-1 text-xs text-gray-400">${escapeHtml(guest.email || guest.mobileNumber || "External invite")}</div></div><div class="rounded-full border px-3 py-1 text-[11px] font-bold ${badgeClass}">${badgeLabel}</div></div><div class="grid grid-cols-2 gap-3 text-xs text-gray-400"><div><span class="font-bold text-gray-300">Invite:</span> ${escapeHtml(guest.inviteStatus || "invited")}</div><div><span class="font-bold text-gray-300">Payment:</span> ${escapeHtml(guest.paymentStatus || "PENDING")}</div></div><div class="flex flex-wrap gap-2">${guest.inviteLink ? `<button type="button" data-action="copy-inline-guest-link" data-invite-link="${escapeHtml(guest.inviteLink)}" class="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-white hover:border-cyan-400/40">Copy Invite Link</button>` : ""}${isPending ? `<button type="button" data-action="pay-guest-ticket" data-ticket-id="${ticket.id}" data-invite-token="${guest.inviteToken}" class="rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-200 hover:bg-emerald-500/20">Pay</button>` : ""}${!isCancelled ? `<button type="button" data-action="cancel-guest-ticket" data-ticket-id="${ticket.id}" data-invite-token="${guest.inviteToken}" class="rounded-xl border border-rose-400/30 bg-rose-500/10 px-3 py-2 text-xs font-bold text-rose-200 hover:bg-rose-500/20">Cancel</button>` : ""}${guest.ticketUrl ? `<a href="${guest.ticketUrl}" target="_blank" rel="noopener" class="rounded-xl border border-fuchsia-400/30 bg-fuchsia-500/10 px-3 py-2 text-xs font-bold text-fuchsia-100 hover:bg-fuchsia-500/20">Open Ticket</a>` : ""}</div></div>`;
    })
    .join("");
}
...
function openBookingModal(postId) {
  ...
  if (!isEnded) mountGuestInviteSection();
...
if (!window.__finalGroupTicketBindingsBound) {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      const payGuestButton = event.target.closest(
        '[data-action="pay-guest-ticket"]',
      );
      if (payGuestButton?.dataset.ticketId && payGuestButton?.dataset.inviteToken) {
        event.preventDefault();
        event.stopImmediatePropagation();
        payGuestInviteByOwner(
          payGuestButton.dataset.ticketId,
          payGuestButton.dataset.inviteToken,
        );
        return;
      }
      const cancelGuestButton = event.target.closest(
        '[data-action="cancel-guest-ticket"]',
      );
      if (
        cancelGuestButton?.dataset.ticketId &&
        cancelGuestButton?.dataset.inviteToken
      ) {
        event.preventDefault();
        event.stopImmediatePropagation();
        cancelGuestInviteByOwner(
          cancelGuestButton.dataset.ticketId,
          cancelGuestButton.dataset.inviteToken,
        );
        return;
      }
      const generateTicketGuestInviteButton = event.target.closest(
        '[data-action="generate-ticket-guest-invite"]',
      );
      if (generateTicketGuestInviteButton?.dataset.ticketId) {
        event.preventDefault();
        event.stopImmediatePropagation();
        generateGuestInviteForTicket(generateTicketGuestInviteButton.dataset.ticketId);
        return;
      }
      const copyInlineGuestLinkButton = event.target.closest(
        '[data-action="copy-inline-guest-link"]',
      );
      if (copyInlineGuestLinkButton?.dataset.inviteLink) {
        event.preventDefault();
        event.stopImmediatePropagation();
        copyTextValue(copyInlineGuestLinkButton.dataset.inviteLink)
          .then(() => setLocationStatus("Guest invite link copied."))
          .catch(() =>
            setLocationStatus("Could not copy the guest invite link.", true),
          );
        return;
